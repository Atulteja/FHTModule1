...
def genPopulation(size, empirical_weights):
    ...
    for _ in range(size - 1):
        new_individual = empirical_weights + np.random.randint(-20, 21, size=len(empirical_weights))
...
def balance_dataset(X, y):
    ...
    X_pos, X_neg = X[mask_pos], X[mask_neg]
    y_pos, y_neg = y[mask_pos], y[mask_neg]

    min_size = min(len(X_pos), len(X_neg))
    if len(X_pos) > min_size:
        X_pos, y_pos = resample(X_pos, y_pos, n_samples=min_size, random_state=42)
    if len(X_neg) > min_size:
        X_neg, y_neg = resample(X_neg, y_neg, n_samples=min_size, random_state=42)

    X_balanced = np.vstack((X_pos, X_neg))
    y_balanced = np.hstack((y_pos, y_neg))
